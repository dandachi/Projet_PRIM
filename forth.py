...
pas=1
Q_hats=[]
mean_correction=np.zeros(len(n_factors))
...
for k_factor in n_factors:
    ...
    mean_correction[n_factors.index(k_factor)]=np.mean(W_test * (Q_test - Q_hat))*(np.size(W_test)/np.sum(W_test))
    Q_hat=Q_hat+W_test*mean_correction[n_factors.index(k_factor)]
    Q_hat=Q_hat.clip(min=0,max=5) 
    Q_hats.append(Q_hat)
    k_errors[n_factors.index(k_factor)]=get_error_Qpred(Q_test,Q_hat,W_test)
#plt.plot(errors);
#plt.ylim([0, 1000000]);
...
for h in range(0,len(n_factors)):
    ...
    plt.savefig('forth/hist'+str(n_factors[h])+'-pen.png')
